<template>
    <div class="yanbiqi">
      <div class="list">
        <div class="col-lg-12 control-section">
          <div class="slider-content-wrapper1">
            <div class="slider_container" style="position: relative">
              <div class="slider-labeltext slider_userselect" style="position: absolute;top: 50%;left: -100px;color:#fff">启动时间（ms）</div>
              <p style="color: #fff;margin-left: 60px">{{time1}}</p>
              <ejs-slider id="gradient_slide1" min=10 max=1000 value=0 type='MinRange' v-model="time1" v-on:changed="end"></ejs-slider>
            </div>
            <div class="slider_container" style="position: relative">
              <div class="slider-labeltext slider_userselect" style="position: absolute;top: 50%;left: -100px;color:#fff">门限 （dB）</div>
              <p style="color:#fff;margin-left: 60px">{{menxian1}}</p>
              <ejs-slider id="gradient_slide1" min=-60 max=0 value=0 type='MinRange' v-model="menxian1" v-on:changed="end"></ejs-slider>
            </div>
          </div>
        </div>
        <div class="col-lg-12 control-section" style="margin-left: 30px">
          <div class="slider-content-wrapper1">
            <div class="slider_container" style="position: relative">
              <div class="slider-labeltext slider_userselect" style="position: absolute;top: 50%;left: -100px;color:#fff">释放时间（ms）</div>
              <p style="color: #fff;margin-left: 60px">{{time2}}</p>
              <ejs-slider id="gradient_slide1" min=10 max=2000 value=0 type='MinRange' v-model="time2" v-on:changed="end"></ejs-slider>
            </div>
            <div class="slider_container" style="position: relative">
              <div class="slider-labeltext slider_userselect" style="position: absolute;top: 50%;left: -100px;color:#fff">降级阶级（dB）</div>
              <p style="color: #fff;margin-left: 60px">{{menxian2}}</p>
              <ejs-slider id="gradient_slide1" min=-100 max=0 value=0 type='MinRange' v-model="menxian2" v-on:changed="end"></ejs-slider>
            </div>
          </div>
        </div>
      </div>
      <div class="bottomSlide" style="margin-top: -10px">
        <div class="col-lg-12 control-section">
          <div class="slider-content-wrapper2">
            <div class="slider_container" style="position: relative">
              <p style="position: absolute;top:20px;left: -55px;color: #fff">关闭</p>
              <p style="position: absolute;top:20px;left: 90px;width: 40px;color: #fff;">开启</p>
              <ejs-slider id="gradient_slide2" min=0 max=1 value=0 type='MinRange' v-model="jianting" v-on:changed="end"></ejs-slider>
            </div>
          </div>
        </div>
      </div>
    </div>
</template>
<script>
    import Vue from 'vue';
    import { SliderPlugin } from "@syncfusion/ej2-vue-inputs";
    Vue.use(SliderPlugin);
    import { enableRipple } from '@syncfusion/ej2-base';
    enableRipple(true);
    export default {
        name: "yan-page",
        data(){
          return{
            time1:100,
            time2:100,
            menxian1:-40,
            menxian2:-60,
            jianting:0,
            IP:'',
            response:[]
          }
        },
        methods:{
          end(){
            if(document.querySelector('.img20').src!='data:image/png;base64,'){
              this.response.Ducker[0].attacktime=this.time1
              this.response.Ducker[0].releasetime=this.time2
              this.response.Ducker[0].threshold= this.menxian1
              this.response.Ducker[0].value=this.menxian2
              this.response.Ducker[0].bypass=-(this.jianting-1)
              localStorage.setItem('response',JSON.stringify(this.response))
              let silence=`<Ducker channel="96" attacktime="${this.time1}" releasetime="${this.time2}" threshold="${this.menxian1}" bypass="${-(this.jianting-1)}">${this.menxian2}</Ducker>`
              let encrypted=this.coding(silence)
              this.$.ajax({
                url: `http://${this.IP}`,
                data:{control:1,commd:encrypted},
                type: "POST",
                dataType:'json',
                error:(err)=>{
                  let IP1=this.IP
                  let silence1=`<Ducker channel="96" attacktime="${this.time1}" releasetime="${this.time2}" threshold="${this.menxian1}" bypass="${-(this.jianting-1)}">${this.menxian2}</Ducker>`
                  let encryped1=this.coding(silence1)
                  if(err.statusText=="error"){
                    console.log("出错了")
                    setTimeout(()=>{
                      this.$.ajax({
                        url: `http://${IP1}`,
                        data:{control:1,commd:encryped1},
                        type: "POST",
                        dataType:'json',
                      })
                    },1500)
                  }
                }
              })
            }
          }
        },
        created(){
          this.IP=localStorage.getItem('ip')
          if(document.querySelector('.img20').src!='data:image/png;base64,'){
            if(localStorage.getItem('response')!=undefined){
              this.response=JSON.parse(localStorage.getItem('response'))
              this.time1=this.response.Ducker[0].attacktime
              this.time2=this.response.Ducker[0].releasetime
              this.menxian1=this.response.Ducker[0].threshold
              this.menxian2=this.response.Ducker[0].value
              this.jianting=-(this.response.Ducker[0].bypass-1)
            }
          }
        },
    }
</script>

<style lang="less">
  @import "../../node_modules/@syncfusion/ej2-vue-inputs/styles/material.css";
  div.yanbiqi{
    height:37%;
    width: 40%;
    margin: 0 auto;
    div.list{
      display: flex;
      flex-wrap: nowrap;
      justify-content: space-around;
      padding-top: 5%;
      div.control-section{
        .slider-content-wrapper1{
          margin: 0 auto;
          min-width:150px;
        }
      }
    }
    div.bottomSlide{
      margin-left:40%;
      div.control-section{
        width: 60px;
        .slider-content-wrapper2{
          min-width: 40px;
        }
      }
    }
  }
  .slider-labeltext {
    text-align: left;
    font-weight: 500;
    font-size: 13px;
    padding-bottom: 10px;
  }
  .slider_container {
   margin-top: 0;
  }
  #gradient_slide1 .e-handle {
    height:30px;
    width: 30px;
    background: url("../assets/core.png");
    background-size:100% 100%;
    border: none;
    background-color: #000;
    top: calc(50% - 10px);
    margin-left:-8px;
  }
  #gradient_slide1 .e-handle:active{
    transform: scale(1);
  }
  #gradient_slide1 .e-range {
    height:15px;
    top: calc(50% - 3px);
    border-radius:10px;
    background-color: #08a2f9;
  }
  #gradient_slide1 .e-slider-track {
    height: 15px;
    top: calc(50% - 4px);
    border-radius: 10px;
  }
  #gradient_slide2 .e-handle {
    height:32px;
    width:32px;
    background: url("../assets/core.png");
    background-size:100% 100%;
    border: none;
    background-color: #000;
    top: calc(50% - 10px);
    margin-left:-14px;
    margin-top: 3px;
  }
  #gradient_slide2 .e-handle:active{
    transform: scale(1);
  }
  #gradient_slide2 .e-range {
    height:24px;
    top: calc(50% - 3px);
    border-radius:10px;
    background-color: #08a2f9;
  }
  #gradient_slide2 .e-slider-track {
    height: 24px;
    top: calc(50% - 4px);
    border-radius:16px;
  }
</style>
