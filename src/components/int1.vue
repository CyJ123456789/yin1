<template>
    <div class="int1">
        <div class="parent">
          <ul class="table">
            <li v-for="(item,index) in vchart">
              <span v-for="(item1,index1) in item">{{item1}}</span>
            </li>
          </ul>
          <div class="vchart">
            <canvas id="myCanvas1" width=700 height=195></canvas>
          </div>
          <div class="in1">
            <div class="col-lg-12 control-section">
              <div class="slider-content-wrapper3">
                <div class="slider_container">
                  <div class="slider-labeltext slider_userselect"></div>
                  <ejs-slider id="gradient_slide7" min=0 max=100 v-model="ditong" type='MinRange' v-on:changed="jiesuan"></ejs-slider>
                </div>
              </div>
            </div>
          </div>
          <ul class="u1">
            <li>6</li>
            <li>3</li>
            <li>0</li>
            <li>-3</li>
            <li>-6</li>
            <li>-9</li>
            <li>-12</li>
            <li>-15</li>
            <li>-18</li>
          </ul>
          <ul class="u2">
            <li>20</li>
            <li>1.4k</li>
            <li>2.8k</li>
            <li>4.3k</li>
            <li>5.7k</li>
            <li>7.1k</li>
            <li>8.5k</li>
            <li>9.9k</li>
            <li>11.4k</li>
            <li>12.8k</li>
            <li>14.2k</li>
            <li>15.6k</li>
            <li>17k</li>
            <li>18.5k</li>
            <li>20k</li>
          </ul>
          <div class="show">
            <p class="p1">
              <span class="showText">
                频率
              </span>
              <span class="showNum" style="color: #fff">
                {{num6}}
                <img src="../assets/plus.png" class="plus" @click="plus">
                <img src="../assets/minus.png" class="minus" @click="minus">
              </span>
            </p>
            <div class="disable">
              <div class="col-lg-12 control-section">
                <div class="slider-content-wrapper20">
                  <div class="slider_container">
                    <div class="slider-labeltext slider_userselect" style="position: absolute;top: 52px;left: 10px;font-size: 1.0rem;color: #fff" >禁用</div>
                    <ejs-slider id="gradient_slide20" min=0 max=1 v-model="disable" type='MinRange' v-on:changed="jiesuan"></ejs-slider>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
    </div>
</template>

<script>
    import disabled from '../assets/int2-img/disabled.png'
    import pitch from '../assets/int2-img/pitch.png'
    import Vue from 'vue';
    import { SliderPlugin } from "@syncfusion/ej2-vue-inputs";
    Vue.use(SliderPlugin);
    import { enableRipple } from '@syncfusion/ej2-base';
    import connect from "./call/connector"
    enableRipple(true);
    export default {
        name: "int1",
        data(){
          return{
            img1:disabled,
            img2:pitch,
            vchart:[
              ['','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''],
              ['','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''],
              ['','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''],
              ['','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''],
              ['','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''],
              ['','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''],
              ['','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''],
              ['','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','',''],
            ],
            width1:700,
            width2:660,
            width3:634,
            ditong:100,
            num6:20000,
            disable:0,
            tongdao1:0,
            judge:false,
            IP:'',
            response:[],
          }
        },
        methods:{
          jiesuan(){
            if(document.querySelector('.img20').src!='data:image/png;base64,'){
              this.response.LPFilter[this.tongdao1].value=parseInt(this.num6)
              this.response.LPFilter[this.tongdao1].bypass=-(this.disable-1)
              localStorage.setItem('response',JSON.stringify(this.response))
              let silence=`<LPFilter channel="${this.tongdao1}" no="2" bypass="${-(this.disable-1)}">${this.num6}</LPFilter>`
              let encrypted=this.coding(silence)
              this.$.ajax({
                url:`http://${this.IP}`,
                data: {control:1,commd:encrypted},
                type: "POST",
                dataType:'json',
                error:(err)=>{
                  let IP=this.IP
                  let silence1=`<LPFilter channel="${this.tongdao1}" no="2" bypass="${-(this.disable-1)}">${this.num6}</LPFilter>`
                  let encryped1=this.coding(silence1)
                  if(err.statusText=="error"){
                    console.log("出错了")
                    setTimeout(()=>{
                      this.$.ajax({
                        url: `http://${IP}`,
                        data:{control:1,commd:encryped1},
                        type: "POST",
                        dataType:'json',
                      })
                    },800)
                  }
                }
              })
            }
          },
          plus(){
            if(this.num6<20000){
              this.num6+=1
              if(document.querySelector('.img20').src!='data:image/png;base64,'){
                this.response.LPFilter[this.tongdao1].value=parseInt(this.num6)
                this.response.LPFilter[this.tongdao1].bypass=-(this.disable-1)
                localStorage.setItem('response',JSON.stringify(this.response))
                let silence=`<LPFilter channel="${this.tongdao1}" no="2" bypass="${-(this.disable-1)}">${this.num6}</LPFilter>`
                let encrypted=this.coding(silence)
                this.$.ajax({
                  url:`http://${this.IP}`,
                  data: {control:1,commd:encrypted},
                  type: "POST",
                  dataType:'json',
                  error:(err)=>{
                    let IP1=this.IP
                    let silence2=`<LPFilter channel="${this.tongdao1}" no="2" bypass="${-(this.disable-1)}">${this.num6}</LPFilter>`
                    let encryped2=this.coding(silence2)
                    if(err.statusText=="error"){
                      console.log("出错了")
                      setTimeout(()=>{
                        this.$.ajax({
                          url: `http://${IP1}`,
                          data:{control:1,commd:encryped2},
                          type: "POST",
                          dataType:'json',
                        })
                      },800)
                    }
                  }
                })
              }
            }
          },
          minus(){
            if(this.num6>20){
              this.num6-=1
              if(document.querySelector('.img20').src!='data:image/png;base64,'){
                this.response.LPFilter[this.tongdao1].value=parseInt(this.num6)
                this.response.LPFilter[this.tongdao1].bypass=-(this.disable-1)
                localStorage.setItem('response',JSON.stringify(this.response))
                let silence=`<LPFilter channel="${this.tongdao1}" no="2" bypass="${-(this.disable-1)}">${this.num6}</LPFilter>`
                let encrypted=this.coding(silence)
                this.$.ajax({
                  url:`http://${this.IP}`,
                  data: {control:1,commd:encrypted},
                  type: "POST",
                  dataType:'json',
                  error:(err)=>{
                    let IP2=this.IP
                    let silence3=`<LPFilter channel="${this.tongdao1}" no="2" bypass="${-(this.disable-1)}">${this.num6}</LPFilter>`
                    let encryped3=this.coding(silence3)
                    if(err.statusText=="error"){
                      console.log("出错了")
                      setTimeout(()=>{
                        this.$.ajax({
                          url: `http://${IP2}`,
                          data:{control:1,commd:encryped3},
                          type: "POST",
                          dataType:'json',
                        })
                      },800)
                    }
                  }
                })
              }
            }
          }
        },
        created(){
          this.IP=localStorage.getItem('ip')
          this.tongdao1=localStorage.getItem('gallery')
          if(document.querySelector('.img20').src!='data:image/png;base64,'){
            if(localStorage.getItem('response')!=undefined){
              this.response=JSON.parse(localStorage.getItem('response'))
              let num7=parseInt(this.response.LPFilter[this.tongdao1].value)
              this.num6=num7
              console.log(this.num6)
              this.ditong=this.num6/200
              this.disable=-(this.response.LPFilter[this.tongdao1].bypass-1)
            }
          }
        },
        mounted(){
          let myCanvas = document.getElementById('myCanvas1');
          let ctx = myCanvas.getContext('2d');
          /*绘制二次贝塞尔曲线*/
          ctx.beginPath();
          ctx.moveTo(this.width1,195);
          ctx.quadraticCurveTo(this.width2,48,this.width3,48);
          ctx.strokeStyle = "#08a2f9";
          ctx.stroke();
          ctx.beginPath();
          ctx.moveTo(this.width3,48);
          ctx.lineTo(0, 48)
          ctx.stroke();
        },
        watch:{
          ditong(val,oldVal){
            if(val==0){
              this.num6=20
            }else if(val==100){
              this.num6=20000
            }else{
              this.num6=val*200
            }
            let myCanvas = document.getElementById('myCanvas1');
            let ctx = myCanvas.getContext('2d');
            ctx.clearRect(0, 0,700, 195);
            ctx.beginPath();
            ctx.moveTo(this.width1-(100-val)*5.95,195);
            ctx.quadraticCurveTo(this.width2-(100-val)*5.95,48,this.width3-(100-val)*5.95,48);
            ctx.strokeStyle = "#08a2f9";
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(this.width3-(100-val)*5.95,48);
            ctx.lineTo(0, 48);
            ctx.stroke();
          },
          disable(val,oldVal){
            this.response.LPFilter[this.tongdao1].value=this.num6
            this.response.LPFilter[this.tongdao1].bypass=-(val-1)
            localStorage.setItem('response',JSON.stringify(this.response))
            if(document.querySelector('.img20').src!='data:image/png;base64,'){
              let silence=`<LPFilter channel="${this.tongdao1}" no="2" bypass="${-(val-1)}">${this.num6}</LPFilter>`
              let encrypted=this.coding(silence)
              this.$.ajax({
                url:`http://${this.IP}`,
                data: {control:1,commd:encrypted},
                type: "POST",
                dataType:'json',
                error:(err)=>{
                  let IP3=this.IP
                  let silence4=`<LPFilter channel="${this.tongdao1}" no="2" bypass="${-(val-1)}">${this.num6}</LPFilter>`
                  let encryped4=this.coding(silence4)
                  if(err.statusText=="error"){
                    console.log("出错了")
                    setTimeout(()=>{
                      this.$.ajax({
                        url: `http://${IP3}`,
                        data:{control:1,commd:encryped4},
                        type: "POST",
                        dataType:'json',
                      })
                    },1500)
                  }
                }
              })
            }
          }
        }
    }
</script>

<style lang="less">
  @import "../../node_modules/@syncfusion/ej2-vue-inputs/styles/material.css";
  *{
    margin: 0;
    padding: 0;
  }
  div.int1 {
    width: 60%;
    height: 100%;
    margin: 0 auto;
    position: relative;
    .parent {
      width: 700px;
      height: 195px;
      position: absolute;
      top: 8%;
      .table{
        width: 700px;
        height: 195px;
        list-style-type: none;
        box-sizing: border-box;
        border-right: 1px solid #fff;
        border-bottom:1px solid #fff;
        li{
          width: 700px;
          height: 24.375px;
          box-sizing: border-box;
          display: flex;
          span{
            height: 24.375px;
            width: 21.2px;
            box-sizing: border-box;
            border-top:1px solid #fff;
            border-left: 1px solid #fff;
          }
        }
      }
      div.vchart {
        width: 700px;
        height: 195px;
        position: absolute;
        top: 0;
        left: 0px;
        z-index:2;
        background-color: transparent;
      }
      .in1{
        div.control-section {
          width: 595px;
          height: 40px;
          position: absolute;
          top: 38px;
          left: 60px;
          z-index: 6;
          .slider-content-wrapper3 {
            margin: 0 auto;
            min-width: 590px;
          }
        }
      }
      .u1 {
        list-style-type: none;
        position: absolute;
        top: -10px;
        left: -30px;
        width: 20px;
        height: 195px;
        li {
          width: 100%;
          height: 24.375px;
          text-align: right;
          color: #fff;
          font-size: 0.93rem;
        }
      }
      .u2 {
        list-style-type: none;
        width: 700px;
        display: flex;
        margin-left: 50px;
        li {
          color: #fff;
          width: 42.4px;
          font-size: .9rem;
        }
      }
      div.show {
        position: absolute;
        top:50%;
        left:105%;
        width: 30%;
        height: 40px;
        line-height: 40px;
        .p1 {
          width: 200px;
          position: relative;
          margin-left: 10px;
          display: flex;
          .showText{
            margin-right: 20px;
            color: #fff;
          }
          .showNum{
            width: 80px;
            height: 35px;
            line-height:35px;
            padding-left: 10px;
            margin-top: 5px;
            background: url("../assets/int2-img/show.png") no-repeat;
            background-size:100% 100%;
            position: relative;
            .plus{
              position: absolute;
              top:6px;
              left: 75%;
            }
            .minus{
              position: absolute;
              top: 20px;
              left: 75%;
            }
          }
        }
        .disable{
          width:60px;
          margin-left: 70px;
        }
      }
    }
  }
  .slider-labeltext {
    text-align: left;
    font-weight: 500;
    font-size: 13px;
    padding-bottom: 10px;
  }
  #gradient_slide7 .e-handle {
    height:25px;
    width: 25px;
    border: none;
    background: url("../assets/light.png");
    background-size:100% 100%;
    border-radius: 50%;
    margin-top:0%;
    top: calc(50% - 10px);
    margin-left:-8px;
  }
  #gradient_slide7 .e-handle:active{
    transform: scale(1);
  }
  #gradient_slide7 .e-range {
    height: 10px;
    top: calc(50% - 4px);
    border-radius:10px;
    background: transparent;
  }
  #gradient_slide7 .e-slider-track {
    height: 10px;
    top: calc(50% - 4px);
    border-radius: 10px;
    background: transparent;
  }
  #gradient_slide20 .e-handle {
    height:32px;
    width:32px;
    background: url("../assets/core.png");
    background-size:100% 100%;
    border: none;
    background-color: #000;
    top: calc(50% - 10px);
    margin-left:-14px;
    margin-top: 3px;
  }
  #gradient_slide20 .e-handle:active{
    transform: scale(1);
  }
  #gradient_slide20 .e-range {
    height:24px;
    top: calc(50% - 3px);
    border-radius:10px;
    background-color: #08a2f9;
  }
  #gradient_slide20 .e-slider-track {
    height: 24px;
    top: calc(50% - 4px);
    border-radius:16px;
  }
</style>
